...
const handleSubmit = async () => {
  ...
  const UserDietData ={
    ...
    selectedDiet : dietData.filter((d) => selectedDietPreferences.includes(d.id)).map((d)=>({id: d.id,name:d.name,category:d.category}))
   ...
   <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6 relative top-15">
     <div className="text-center space-y-4">
       ...
       <p className="text-muted-foreground max-w-2xl mx-auto text-green-600 text-sm sm:text-base">
         Tell us about your goals and preferences to get a personalized diet plan
...
<Card className="border border-black/10 bg-card shadow-sm">
  <CardHeader>
    ...
    <CardDescription className="text-muted-foreground">
      Choose all preferences that match your dietary goals and restrictions
    </CardDescription>
  </CardHeader>
  <CardContent>
    <motion.div
...
<input
  ...
  onChange={(e) =>
    handleDietPreferenceChange(dietOption.id, e.target.checked)
  }
...
<div className="flex justify-center p-10">
    <button
        ...
        disabled={selectedDietPreferences.length === 0 || !userDetails.name || !userDetails.age}
