...
// Corrected import path assuming LanguageSwitcher is in the same folder or a dedicated components folder
  ...
  return (
    <header className="bg-white shadow-md py-4 px-6 flex items-center justify-between sticky top-0 z-50">
      ...
      <div className="flex items-center gap-4">
        <img src={Logo2} alt="Logo 2" className="w-10" />
        <img src={Logo1} alt="Logo 1" className="w-20" />
      </div>
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-16 left-0 w-full bg-white shadow-lg md:hidden z-40">
          <nav className="flex flex-col space-y-2 p-4">
            <Link to="/" className="text-gray-700 hover:text-blue-500 py-2 text-center" onClick={closeMobileMenu}>
              {t('header.navigation.home')}
            </Link>
            <Link to="/type-input" className="text-gray-700 hover:text-blue-500 py-2 text-center" onClick={closeMobileMenu}>
              {t('header.navigation.symptoms')}
            </Link>
            <Link to="/find-doctor" className="text-gray-700 hover:text-blue-500 py-2 text-center" onClick={closeMobileMenu}>
                {t('header.navigation.findDoctor')}
            </Link>
            <Link to="/WellnessJourneyPage" className="text-gray-700 hover:text-blue-500 py-2 text-center" onClick={closeMobileMenu}>
              {t('header.navigation.fitness')}
            </Link>
            ...
            <div className="pt-2 border-t border-gray-200">
              {user ? (
                ...
                <button
                  onClick={() => { loginPage(); closeMobileMenu(); }}
